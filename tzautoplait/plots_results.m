...
function plot_da(org)
    hold on;
    cols=get(0,'DefaultAxesColorOrder');    
    plot(org(:,1),'color',cols(5,:));
    plot(org(:,2),'color',cols(1,:));
    plot(org(:,3),'color',cols(2,:));    
    legend('Electric load','Cooling load','Heating load');
    xlabel('Time/h')
    ylabel('Power/p.u.')
    box on
end
...
function [  ] = plot_lines( X )
    ...
    hold on;    
    for i=1: length(X)
        plot([X(i), X(i)], [btm, top], 'black-','HandleVisibility','off')
    end
    ylim([btm, top])
end
